...
class Variation_Swatches extends Base {
	...
	public function admin_script() {
		wp_enqueue_media();
		wp_enqueue_script( 'variation-swatches-admin-js', plugins_url( 'js/admin.js', __FILE__ ), array( 'jquery' ), $this->version, true );
	...
	public function front_script() {
		wp_enqueue_script( 'variation-swatches-front-js', plugins_url( 'js/front.js', __FILE__ ), array( 'jquery' ), $this->version, true );
	...
	public function custom_attribute_field() {
		...
		foreach ( $product_taxonomy as $_taxonomy => $value ) {
			...
			add_filter(
				"manage_edit-{$_taxonomy}_columns",
				function ( $columns ) use ( $_taxonomy ) {
			...
			add_filter(
				"manage_{$_taxonomy}_custom_column",
				function ( $columns, $column, $term_id ) {
	...
	public function widget_controls( $widget ) {
		if ( Helper::get_option( 'codesigner_variation_swatches', 'vs-archive-widget' ) == '' || $widget->id == 'shop-beauty' || $widget->id == 'shop-table' || $widget->id == 'shop-flip' || $widget->id == 'shop-minimal' || $widget->id == 'shop-smart' ) {
			return;
		}
