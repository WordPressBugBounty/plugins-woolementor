...
class Admin extends Base {
	...
	public function upgrade() {
		...
		if ( $this->version == ( $old_version = get_option( "{$this->slug}_db-version" ) ) ) {
			return;
		}
	...
	public function setting_navs_add_item( $settings ) {
		...
		if ( ! wcd_is_pro_activated() && $settings->config['id'] == 'codesigner' ) {
			?>
			<li><a href="<?php echo esc_url( $pro_link ); ?>"><?php esc_html_e( 'Get Pro', 'codesigner' ); ?></a></li>
			<?php
		}
	}
	...
	function get_dismissed_notices( $notice_name = '' ) {
		...
		if ( empty( $notice_name ) ) {
			return $notices;
		} elseif ( empty( $notices[ $notice_name ] ) ) {
				return false;
		} else {
			return true;
		}
